...
Page({
  ...
  onLoad: function (options) {
    ...
    // 当前音乐是否在播放
    if (
      appInst.globalData.isMusicPlay &&
  ...
  changePlayState(state) {
    ...
    // 修改全局播放音乐的状态
    appInst.globalData.isMusicPlay = state;
  },
  ...
  handleMusicPlay() {
    ...
    this.musciControl(isPlay, musicId, musicLink);
  },

  // 控制音乐播放/暂停的功能函数
  async musciControl(isPlay, musicId, musicLink) {
    if (isPlay) {
      // 播放
      if (!musicLink) {
        // 获取音乐的播放链接
